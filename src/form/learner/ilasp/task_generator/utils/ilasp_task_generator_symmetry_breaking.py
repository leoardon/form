...
def generate_symmetry_breaking_statements(num_states, accepting_state, rejecting_state, observables,
                                          symmetry_breaking_method, max_disj_size, learn_acyclic, types):
    if symmetry_breaking_method == "increasing-path":
        ...
        return _generate_bfs_symmetry_breaking(num_states, accepting_state, rejecting_state, observables,
                                               symmetry_breaking_method, max_disj_size, types)
    else:
...
def _generate_bfs_symmetry_breaking(num_states, accepting_state, rejecting_state, observables, symmetry_breaking_method,
    ...
    stmt += _generate_injected_observable_statements(observables, types)
...
def _generate_injected_observable_statements(observables, types):
    obs_list = sorted(observables)
    stmts = ["injected_obs_id(%s, %d)." % (obs_list[i], i + 1) for i in range(len(obs_list))]
    stmts.append("injected_num_obs(%d)." % len(obs_list))

    fol_list = list(chain.from_iterable([f"pred(a_{p})", f"pred(e_{p})"] for p in types.keys()))
    stmts += ["injected_fol_id(%s, %d)." % (fol_list[i], i + 1) for i in range(len(fol_list))]
    stmts.append("injected_num_fol(%d)." % len(fol_list))

    stmts.append("injected_valid_label(1..%d)." % (2 * len(obs_list) + 2 * len(fol_list)))
...
def _generate_bfs_symmetry_breaking_rules_alternative(max_disj_size):
    ...
    stmts.append("injected_label(X, (Y, E), OID) :- injected_pos_obs(X, Y, E, O), injected_obs_id(O, OID).")
    stmts.append("injected_label(X, (Y, E), OID) :- injected_pos_fol(X, Y, E, O), injected_fol_id(O, OID).")
    stmts.append("injected_label(X, (Y, E), OID+N) :- injected_neg_obs(X, Y, E, O), injected_obs_id(O, OID), injected_num_obs(N).")
    stmts.append("injected_label(X, (Y, E), OID+N) :- injected_neg_fol(X, Y, E, O), injected_fol_id(O, OID), injected_num_fol(N).")
