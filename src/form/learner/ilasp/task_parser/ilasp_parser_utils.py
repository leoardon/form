...
from ..ilasp_common import CONNECTED_STR, N_TRANSITION_STR, OBS_STR, PRED_STR
...
_PREDICATE_REGEX = re.compile("(?P<neg>not |)(?P<pred>\w+)_at\(")
...
def _parse_negative_transition_body(transition_body_str):
    ...
    for fluent in fluents:
        ...
        if fluent.startswith(POS_FLUENT_STR_PREFIX):
            ...
            m = _PREDICATE_REGEX.match(fluent)
            if m:
                groups = m.groupdict()
                if groups.get("neg"):
                    neg_fluents.append(f"{PRED_STR}({groups['pred']})")
                else:
                    pos_fluents.append(f"{PRED_STR}({groups['pred']})")
