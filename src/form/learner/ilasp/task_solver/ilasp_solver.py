...
def solve_ilasp_task(
    ...
    with open(output_filename, "w") as f:
        ...
        binary_folder_name = binary_folder_name or os.environ.get("ILASP_BINARY_FOLDER")
        ...
        if binary_folder_name is not None:
            ...
            clingo_binary = (
                CLINGO_MINIMAL_BINARY_NAME if compute_minimal else CLINGO_BINARY_NAME
            )
            arguments.append(
                '"' + os.path.join(binary_folder_name, clingo_binary) + '"'
            )

        if operation == ILASP_OPERATION_SEARCH_SPACE:
            arguments.append("-s")

        return_code = subprocess.call(arguments, stdout=f, timeout=timeout)
